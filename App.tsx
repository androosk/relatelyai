  ...
  return (
    <Tab.Navigator
      screenOptions={({ route }) => ({
        ...
        tabBarActiveTintColor: isDark ? '#B36A82' : '#E58DAB', // Primary color
        tabBarInactiveTintColor: isDark ? '#B3B3B8' : '#666666', // Text medium
        tabBarStyle: {
          backgroundColor: isDark ? '#2A2A2E' : '#FFFFFF', // Surface color
          borderTopColor: isDark ? '#3A3A3F' : '#DDDDDD', // Border color
        },
      })}>
      <Tab.Screen name="Home" component={HomeScreen} options={{ headerShown: false }} />
      <Tab.Screen name="Assessment" component={QuizStackNavigator} />
  ...
  if (loading) {
    return (
      <View
        ...
        style={{ backgroundColor: isDark ? '#1F1F23' : '#F4F1DE' }}>
        <ActivityIndicator size="large" color={isDark ? '#B36A82' : '#E58DAB'} />
      </View>
    );
  }
  ...
  return (
    <NavigationContainer
      theme={{
        dark: isDark,
        colors: {
          primary: isDark ? '#B36A82' : '#E58DAB',
          background: isDark ? '#1F1F23' : '#F4F1DE',
          card: isDark ? '#2A2A2E' : '#FFFFFF',
          text: isDark ? '#EEEEEE' : '#333333',
          border: isDark ? '#3A3A3F' : '#DDDDDD',
          notification: isDark ? '#D99A4B' : '#FFB85F',
        },
        fonts: {
          regular: {
            fontFamily: 'System',
